...
class TestUserManagement(TestCase):
    ...
    def setUp(self): 
        ...
        self.client = Client() 
        self.superuser = self.user_model.objects.create_superuser('django2', 'django2@geekshop.local', 'geekbrains')

        self.user = self.user_model.objects.create_user('tarantino', 'tarantino@geekshop.local', 'geekbrains')

        self.user_with__first_name = self.user_model.objects.create_user('umaturman', 'umaturman@geekshop.local', 'geekbrains', first_name='Ума')
    ...
    def test_user_not_logged(self):
        ...
        self.assertEqual(response.context['page_title'], 'main')
    ...
    def test_user_login(self):
        # данные пользователя
        # self.client.post('/auth/login/', data={'username':'django', 'password':'geekbrains'})
        ...
        response = self.client.get('/')
        self.assertContains(response, self.user, status_code=200)
        self.assertEqual(response.context['user'], self.user)
    ...
    def test_user_register(self):
       ...
       self.assertEqual(response.status_code, 200)
       self.assertEqual(response.context['page_title'], 'registration')
       self.assertTrue(response.context['user'].is_anonymous)
       ...
       response = self.client.post('/auth/reg/', data=new_user_data)
       self.assertEqual(response.status_code, 302)
    ...
    def test_user_wrong_register(self):
        ...
        response = self.client.post('/auth/reg/', data=new_user_data)
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'reg_form', 'age', 'You must be over 18 years old')
